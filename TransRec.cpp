...
private:
	...
	vector<vector<vector<double> > > A, A_tmp, M,M_tmp;
	int label_vec_size = 27;
	vector<vector<double>> label_vec;
	...
	void gradient_one(int e1, int e2, int rel, int belta)
	{
		for (int ii = 0; ii<m; ii++)
		{
			...
			for (int jj = 0; jj<label_vec_size; jj++)
			{
				tmp3 += M[rel][jj][ii] * label_vec[e1][jj];
			}
			for (int jj = label_vec_size; jj<2*label_vec_size; jj++)
			{
				tmp3 += M[rel][jj][ii] * label_vec[e2][jj- label_vec_size];
			}

			double x = 2 * (tmp2 - tmp1 - relation_vec[rel][ii]-tmp3);
			if (L1_flag)
				if (x>0)
					x = 1;
				else
			...
			for (int jj = 0; jj<label_vec_size; jj++)
			{
				M_tmp[rel][jj][ii] -= belta*rate*x*label_vec[e1][jj];
			}
			for (int jj = label_vec_size; jj<2 * label_vec_size; jj++)
			{
